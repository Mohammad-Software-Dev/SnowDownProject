...
func _init() -> void:
	var idle := FirstPersonArmsPose.resolve(SnowballActionComponent.HANDS_FREE, 0.0, 0.0, 0, 0.0, 0.0)
	_assert_true(not bool(idle["ball_visible"]), "idle without inventory hides ball")

	var hold := FirstPersonArmsPose.resolve(SnowballActionComponent.HANDS_FREE, 0.0, 0.0, 2, 0.0, 0.0)
	_assert_true(bool(hold["ball_visible"]), "inventory exposes hold ball")

	var packing_early := FirstPersonArmsPose.resolve(SnowballActionComponent.PACKING, 0.2, 0.0, 0, 0.0, 0.0)
	var packing_late := FirstPersonArmsPose.resolve(SnowballActionComponent.PACKING, 0.9, 0.0, 0, 0.0, 0.0)
	...
	var charge_low := FirstPersonArmsPose.resolve(SnowballActionComponent.THROW_CHARGING, 0.0, 0.1, 1, 0.0, 0.0)
	var charge_high := FirstPersonArmsPose.resolve(SnowballActionComponent.THROW_CHARGING, 0.0, 1.0, 1, 0.0, 0.0)
	...
	var catch_pose := FirstPersonArmsPose.resolve(SnowballActionComponent.CATCHING, 0.0, 0.0, 1, 0.0, 0.0)
	...
	var reaction := FirstPersonArmsPose.resolve(SnowballActionComponent.HANDS_FREE, 0.0, 0.0, 0, 0.0, 1.0)
