...
func _process(_delta: float) -> void:
	...
	var lines: Array[String] = ["Snowdown DEV", "role: %s" % App.runtime_role, "world: %s" % App.active_world, "scenario: %s" % App.active_scenario, "fps: %d" % Engine.get_frames_per_second()]
	...
	if network_session != null:
		...
		lines.append("sim: %d ms  jitter=%d ms  loss=%.1f%%" % [net["sim_latency_ms"], net["sim_jitter_ms"], net["sim_loss_percent"]])
		...
		lines.append("catch: success=%s rewind=%.1f ms" % [net["catch_succeeded"], net["catch_rewind_ms"]])
		lines.append("server score: A %d — %d B" % [net["team_a_score"], net["team_b_score"]])
		if not String(net["last_event"]).is_empty(): lines.append("last net event: %s" % net["last_event"])
	...
	if gameplay_world != null:
		...
		if gameplay_world.has_method("get_world_debug_snapshot"):
			...
			if world_snapshot.has("nearest_snow_distance") and float(world_snapshot["nearest_snow_distance"]) >= 0.0: lines.append("nearest snow: %.1f m" % float(world_snapshot["nearest_snow_distance"]))
